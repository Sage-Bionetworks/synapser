...
**Important:** Synapser requires Python 3.9 to 3.11 (we recommend Python 3.10). For detailed installation instructions see [installation vignette](installation.html). For troubleshooting dependency conflicts, please refer to the [Troubleshooting](troubleshooting.html) guide.
...
act <- Activity(
  ...
  used = list(UsedEntity(target_id = "syn1234"), UsedEntity(target_id = "syn1235")),
  executed = list(UsedEntity(target_id = "syn4567")))
```
...
act$used <- list(UsedEntity(target_id = "syn12345"), UsedEntity(target_id = "syn12346"))
act$executed <- list(UsedEntity(target_id = "syn4567"))
```

The used and executed lists can reference entities in Synapse (`UsedEntity`) or URLs (`UsedURL`). They accept only `UsedEntity` and `UsedURL` objects. Other values, such as a plain Synapse ID string, are dropped with no error.

Entity examples:
```{r collapse = TRUE}
  UsedEntity(target_id = "syn12345")
  UsedEntity(target_id = project$id)
  UsedEntity(target_id = "syn12345", target_version_number = 2L)
```

URL examples:
```{r collapse = TRUE}
  UsedURL(url = "http://mydomain.com/my/awesome/data.RData")
  UsedURL(name = "Awesome Data", url = "http://mydomain.com/my/awesome/data.RData")
```

To record code that was executed, put it in the executed list:
```{r collapse = TRUE}
act$executed <- list(UsedURL(name = "Gnarly hacks", url = "https://github.com/joe_hacker/code_repo"))
```

### Storing entities with provenance

The activity can be set when storing an Entity to set the Entity’s provenance:

```{r collapse = TRUE}
filePath <- tempfile()
writeLines("clustering results", filePath)
file <- File(path = filePath, parent_id = project$id, activity = act) |> synStore()
```

We’ve now recorded that 'file' is the output of the code in the executed list applied to the data stored in syn12345 and syn12346.

### Getting the provenance of an entity

```{r collapse = TRUE}
act <- Activity |> synFromParent(parent = file)
```

You can also get the activity together with the entity:

```{r collapse = TRUE}
file <- synGet(file$id, activity_options = ActivityOptions(include_activity = TRUE))
file$activity
```

### Recording data source

When storing a data entity, it’s a good idea to record its source:
...
file <- file |>
  synSetProperties(
    activity = Activity(
      name = "data-r-us",
      description = "downloaded from data-r-us",
      used = list(UsedURL(url = "http://data-r-us.com/excellent/data.xyz")))
  ) |>
  synStore()
```
...
To remove the link between an entity and its activity:

```{r collapse = TRUE}
Activity |> synDisassociateActivityFromEntity(parent = file)
```
...
?Activity
?ActivityOptions
?Activity_synFromParent
?Activity_synDisassociateActivityFromEntity
```
...
[REST API documentation](<https://rest-docs.synapse.org/rest/).
...
[Synapse User Guide](https://docs.synapse.org/synapse-docs).
