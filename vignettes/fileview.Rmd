...
data
```
To update 'class' annotation for 'file2', simply updating the view:
```{r collapse=TRUE, eval=F}
data['class'] = c('V', 'VI')
saveToCsv(data, queryResults$filepath)
synStore(Table(view$properties$id, queryResults$filepath))
```
