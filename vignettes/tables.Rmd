...
new_schema <- synStore(Schema(name='Jazz Albums', columns=new_cols, parent=project))
...
new_data["covers"] <- fileHandleIds

temp_file<-tempfile()
write.csv(new_data, file=temp_file, row.names=FALSE)
synStore(Table(new_schema$properties$id, temp_file))
```

## Set Annotations
A table is a Synapse entity. Annotations on table works the same way with annotations on any other entity types. Please visit [synapser vignettes](synapser.html) to read more about how to set annotations on an entity.
