...
To update table with your changes, retrieve the data.frame with ROW_ID and ROW_VERSION as columns before making changes to the data.frame:

```{r collapse=TRUE}
df <-results$asDataFrame(rowIdAndVersionInIndex=FALSE)
df["TranscriptionFactor"]=TRUE
write.csv(df, results$filepath, row.names=FALSE)
```

Let’s save that:
```{r collapse=TRUE}
table<-Table(results$tableId, results$filepath)
table<-synStore(table)
```

Now, query the table again to see your changes:
```{r collapse=TRUE}
results <- synTableQuery(sprintf("select * from %s limit 10", results$tableId))
results$asDataFrame(rowIdAndVersionInIndex=FALSE)
```
